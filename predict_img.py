...
def run_testing_on_dataset(trained_model, dataset_dir, GT_blurry):
    ...
    for ind, image_name in enumerate(img_list_bar):
        # Read the image
        img = cv2.imdecode(np.fromfile(os.path.join(dataset_dir, image_name), dtype=np.uint8), 0)
...
if __name__ == '__main__':
    ...
    dataset_dir = '/lustre/home/xyang/datasets/模糊数据测试数据/模糊/'
    accuracy_blurry_images = run_testing_on_dataset(trained_model, dataset_dir, GT_blurry=True)

    dataset_dir = '/lustre/home/xyang/datasets/模糊数据测试数据/清晰/'
    accuracy_sharp_images = run_testing_on_dataset(trained_model, dataset_dir, GT_blurry=False)

    dataset_dir = '/lustre/home/xyang/datasets/模糊数据测试数据/中间/'
    accuracy_middle_images = run_testing_on_dataset(trained_model, dataset_dir, GT_blurry=False)
    
    print("=====================================")
    print(f'Accuracy on blurry images: {accuracy_blurry_images}')
    print(f'Accuracy on sharp images: {accuracy_sharp_images}')
    print(f'Accuracy on middle images: {accuracy_middle_images}')